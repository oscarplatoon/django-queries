...
class ProductCrud:
    ...
    def last_record(cls):
        ...
        #[e for e in Product.objects.all()][-1]
    ...
    def by_rating_range(cls, rating_low, rating_high):
        return Product.objects.filter(rating__range = (rating_low,rating_high))
    ...
    def manufacturer_names_for_query(cls, input):
        return Product.objects.filter(manufacturer__icontains = input).values_list('manufacturer',flat=True)
